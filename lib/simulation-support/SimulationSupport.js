...
SimulationSupport.prototype.getElementTrigger = function(id) {
  return this._bpmnjs.invoke(['bpmnjs', function(bpmnjs) {
    return domQuery(
      `.djs-overlays[data-container-id*='${id}'][data-container-id*='delta'] .bts-context-pad:not(.hidden)`,
      bpmnjs._container
    );
  }]);
};
...
SimulationSupport.prototype.elementEnter = function(elementType = null) {
  return new Promise(resolve => {
    return this._bpmnjs.invoke(['eventBus', function(eventBus) {
      const wrap = elementType ? (fn) => ifElementHasType(elementType, fn) : fn => fn;
...
function ifElementHasType(elementType, fn) {
  return function(event) {
    if (event.element && event.element.type === elementType) {
      return fn(event);
    }
  };
}
